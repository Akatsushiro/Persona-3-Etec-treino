...
$extensao = $separa[0];
$imagemF = tratamento_de_espacos($nome) . "." . $extensao;
...
if (move_uploaded_file($_FILES['imagem']['tmp_name'], $imgDir . $imagemF)) {
    ...
    */
    ...
    são divididas em tipos, cada tipo terá umas listagem por checkbox
    */
    while ($i2 < count(habilidades)) {
        $sql->query("INSERT INTO `persona_habilidade`(`id_persona_hab`, `id_habilidade_per`) 
		VALUES('$last_id',$habilidades[i])");
        $i2++;
    }
    } else {
